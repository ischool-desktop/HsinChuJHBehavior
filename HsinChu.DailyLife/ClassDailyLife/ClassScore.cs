...
namespace HsinChu.DailyLife.ClassDailyLife
{
    public partial class ClassScore : BaseForm
    {
        ...
        private void GetConfigData()
        {
            ...
            if (cd.Contains("OtherRecommend") && cd["OtherRecommend"] != "")
            {
                XmlElement xml = DSXmlHelper.LoadXml(cd["OtherRecommend"]);
                cboPrefs.Items.Add(xml.GetAttribute("Name"));

                List<string> list = new List<string>();
                list.Add(xml.GetAttribute("Name"));

                DicSetup.Add("OtherRecommend", list);
            }
        ...
        private void ColumnCheng()
        {
            ...
            else if (cboPrefs.SelectedIndex == 1) //OtherRecommend
            {
                foreach (string each in DicSetup["OtherRecommend"])
                {
                    SetColumnNameLock(each, 400);
                }
            }
            else if (cboPrefs.SelectedIndex == 2) //DailyLifeRecommend
            {
            ...
            foreach (JHStudentRecord student in MSList.StudentRe)
            {
                ...
                if (StudentSR != null)
                {
                    ...
                    else if (cboPrefs.SelectedIndex == 1) //OtherRecommend
                    {
                        ...
                        XmlElement StudentDBXml = StudentSR.TextScore.SelectSingleNode("OtherRecommend") as XmlElement;
                        if (StudentDBXml != null)
                        {
                            foreach (string db in DicSetup["OtherRecommend"])
                            {
        ...
        private void SaveData()
        {
            ...
            foreach (DataGridViewRow each in dataGridViewX1.Rows)
            {
                ...
                else if (cboPrefs.SelectedIndex == 1)
                {
                    ...
                    if (cd.Contains("OtherRecommend"))
                    {
                        ...
                        //讀取設定值內容
                        XmlElement node = DSXmlHelper.LoadXml(cd["OtherRecommend"]);

                        //設定此Element的名稱
                        hlptextscore.GetElement("OtherRecommend").SetAttribute("Name", node.GetAttribute("Name"));

                        XmlElement qnode = hlptextscore.GetElement("OtherRecommend");
                        string ColumnName = qnode.GetAttribute("Name");
                        if (ColumnIndex.ContainsKey(ColumnName))
                        {
                            int ColumnItemIndex = ColumnIndex[ColumnName];
                            hlptextscore.GetElement("OtherRecommend").SetAttribute("Description", "" + each.Cells[ColumnItemIndex].Value);
                        }
                    }
                    #endregion
                }
            ...
            if (UPInsertMSR.Count != 0)
            {
                ...
                sb.AppendLine("由「評等輸入」功能，將學生「日常生活表現資料」進行批次修改。");
        ...
        private StringBuilder SetLog(string IsUpdataOrInsert, JHMoralScoreRecord sbEach)
        {
            ...
            {
                XmlElement xml1 = (XmlElement)sbEach.TextScore.SelectSingleNode("OtherRecommend");
                if (xml1 != null)
                {
                    sc.Append(IsUpdataOrInsert + "「" + xml1.GetAttribute("Name") + "」");
